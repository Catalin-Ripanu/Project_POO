...
public class Warrior extends Character {
    ...
    public Warrior(String heroName, int exp, int level) {
        super(heroName, exp, level);
        setIncreaseStrength(2);
    ...
    public void receiveDamage(int damage) {
        ...
        boolean chance = random.nextInt(1, 151 - (getCharisma() + getDexterity())) <= 20;
        if (chance) {
            if (damage % 2 == 0) {
    ...
    public int getDamage() {
        SplittableRandom random = new SplittableRandom();
        int damage = 25 + getStrength()/2 + getCurrLevel() + random.nextInt(1, getStrength()) / 5;
        boolean chance = random.nextInt(1, 135 - getStrength()) <= 10;
        if (chance) {
            return 2 * damage;
        }
        return damage;
